...
static int get_in_samples_planar_string(swr_t *swr, value *in_vector,
    ...
    for (i = 0; i < swr->in.nb_channels; i++) {
        ...
        if (str_len !=
            caml_string_length(str) - offset * swr->in.bytes_per_samples)
...
static int get_in_samples_ba(swr_t *swr, value *in_vector, int offset) {
    ...
    swr->in.data[0] =
        Caml_ba_data_val(*in_vector) + offset * swr->in.nb_channels;
    CAMLreturnT(int, nb_samples);
}
...
swr_t *swresample_create(vector_kind in_vector_kind,
    ...
    SwrContext *ctx = swresample_set_context(
        swr, in_channel_layout, in_sample_fmt, in_sample_rate,
        out_channel_layout, out_sample_fmt, out_sample_rate, options);
...
CAMLprim value ocaml_swresample_create(
    ...
    CAMLxparam4(_out_channel_layout, _out_sample_fmt, _out_sample_rate,
                _options);
    CAMLlocal1(ans);
    ...
    AVChannelLayout *in_channel_layout =
        AVChannelLayout_val(_in_channel_layout);
