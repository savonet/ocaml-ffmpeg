...
CAMLprim value ocaml_av_get_stream_avg_frame_rate(value _stream) {
    ...
    value_of_rational(&av->format_context->streams[index]->avg_frame_rate,
                      &ans);
...
CAMLprim value ocaml_av_set_stream_time_base(value _stream, value _time_base) {
    ...
    av->format_context->streams[index]->time_base =
...
static int ocaml_avio_read_callback(void *private, uint8_t *buf, int buf_size) {
    ...
    res = caml_callback3_exn(avio->read_cb, buffer, Val_int(0),
                             Val_int(buf_size));
...
static int ocaml_avio_write_callback(void *private, const uint8_t *buf,
    ...
    res = caml_callback3_exn(avio->write_cb, buffer, Val_int(0),
                             Val_int(buf_size));
    if (Is_exception_result(res)) {
        res = Extract_exception(res);
...
CAMLprim value ocaml_av_create_io(value bufsize, value _read_cb,
    ...
    avio->avio_context =
        avio_alloc_context(buffer, buffer_size, write_flag, (void *)avio,
...
CAMLprim value ocaml_av_open_input_stream(value _avio, value _format,
    ...
    // open input format
    av_t *av =
        open_input(NULL, format, avio->format_context, Val_none, &options);
...
static int decode_packet(av_t *av, stream_t *stream, AVPacket *packet,
    ...
    if (dec->codec_type == AVMEDIA_TYPE_AUDIO ||
        ...
        ret = avcodec_decode_subtitle2(dec, (AVSubtitle *)frame,
                                       &stream->got_frame, packet);
        if (ret >= 0) {
            av_packet_unref(packet);
            caml_acquire_runtime_system();
            return ret;
    ...
    do {
        if (!av->end_of_file && !av->frames_pending) {
            ...
            for (i = 0; i < Wosize_val(_packet); i++)
                if (Int_val(Field(Field(_packet, i), 0)) ==
                    packet->stream_index) {
                    skip = 0;
                }

            for (i = 0; i < Wosize_val(_frame); i++)
                if (Int_val(Field(Field(_frame, i), 0)) ==
                    packet->stream_index) {
                    ...
                    if ((stream = streams[packet->stream_index]) == NULL)
                        stream =
                            open_stream_index(av, packet->stream_index, dec);
            ...
            for (i = 0; i < Wosize_val(_packet); i++) {
                if (Int_val(Field(Field(_packet, i), 0)) ==
                    ...
                    Store_field(decoded_content, 0,
                                Val_int(packet->stream_index));
                    Store_field(decoded_content, 1,
                                value_of_ffmpeg_packet(packet));
...
CAMLprim value ocaml_av_seek_native(value _flags, value _stream, value _min_ts,
    ...
    if (_min_ts != Val_none) {
        min_ts =
            (Int64_val(Field(_min_ts, 0)) * num) / (den * second_fractions);
    }

    if (_max_ts != Val_none) {
        max_ts =
            (Int64_val(Field(_max_ts, 0)) * num) / (den * second_fractions);
    }
    ...
    ret = avformat_seek_file(av->format_context, index, min_ts, timestamp,
                             max_ts, flags);
...
CAMLprim value ocaml_av_output_format_guess(value _short_name, value _filename,
    ...
    if (caml_string_length(_short_name) > 0) {
        short_name = av_strndup(String_val(_short_name),
                                caml_string_length(_short_name));
        if (!short_name)
            caml_raise_out_of_memory();
    };
...
static av_t *open_output(avioformat_const AVOutputFormat *format,
    ...
    if (avio_context) {
        ...
        if (!(av->format_context->oformat->flags & AVFMT_NOFILE)) {
            caml_release_runtime_system();
            int err = avio_open2(&av->format_context->pb, file_name,
                                 AVIO_FLAG_WRITE, interrupt_cb_ptr, options);
            caml_acquire_runtime_system();
    ...
    if (stream->codecpar->codec_id == AV_CODEC_ID_H264) {
        ...
        while (data && (data - stream->codecpar->extradata + 19) <
            ...
            if (!(data[0] | data[1] | data[2]) && data[3] == 1 &&
                ...
                rbsp_buf = ocaml_av_ff_nal_unit_extract_rbsp(data, remain_size,
                                                             &rbsp_size, 0);
                if (!rbsp_buf)
                    goto fail;
                if (rbsp_size < 13) {
        ...
        if (stream->codecpar->codec_tag == MKTAG('h', 'v', 'c', '1') &&
            ...
            snprintf(attr, sizeof(attr), "%s.%d.4.L%d.B01",
                     av_fourcc2str(stream->codecpar->codec_tag), profile,
                     level);
        } else
