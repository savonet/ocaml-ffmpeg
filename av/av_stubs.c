...
static stream_t *allocate_stream_context(av_t *av, int index, AVCodec *codec)
{
  ...
  if (type != AVMEDIA_TYPE_AUDIO && type != AVMEDIA_TYPE_VIDEO && type != AVMEDIA_TYPE_SUBTITLE)
    Fail("Failed to allocate stream %d of media type %s", index, av_get_media_type_string(type));
...
static int decode_packet(av_t * av, stream_t * stream, AVPacket * packet, AVFrame * frame)
{
...
CAMLprim value ocaml_av_output_format_guess(value _short_name, value _filename, value _mime)
{
  ...
  if (caml_string_length(_short_name) > 0) {
    short_name = strndup(String_val(_short_name), caml_string_length(_short_name));
    if (!short_name) caml_raise_out_of_memory();
  };
...
static av_t *open_output(AVOutputFormat *format, char *file_name, AVIOContext *avio_context, AVDictionary **options)
{
  int ret;
  av_t *av = (av_t*)calloc(1, sizeof(av_t));
...
static void init_stream_encoder(av_t *av, stream_t *stream, AVDictionary **options)
{
...
static stream_t *new_audio_stream(av_t *av, enum AVSampleFormat sample_fmt, AVCodec *codec, AVDictionary **options)
{
...
CAMLprim value ocaml_av_new_audio_stream(value _av, value _sample_fmt, value _codec, value _opts)
{
  ...
  stream_t * stream = new_audio_stream(Av_val(_av), Int_val(_sample_fmt), codec, &options);
...
static stream_t *new_video_stream(av_t *av, AVCodec *codec, AVDictionary **options)
{
  stream_t *stream = new_stream(av, codec);
...
static stream_t *new_subtitle_stream(av_t *av, AVCodec *codec, AVDictionary **options)
{
  stream_t * stream = new_stream(av, codec);
  if (!stream) return NULL;
...
static void write_frame(av_t * av, int stream_index, AVCodecContext * enc_ctx, AVFrame * frame)
{
  AVStream * avstream = av->format_context->streams[stream_index];
  int ret;
...
static void write_subtitle_frame(av_t * av, int stream_index, AVSubtitle * subtitle)
{
  stream_t * stream = av->streams[stream_index];
