...
CAMLprim value ocaml_av_open_input(value _url)
{
  ...
  free(url);
  caml_acquire_runtime_system();
  if( ! av) ocaml_avutil_raise_error(lasterr);
...
CAMLprim value ocaml_av_open_input_stream(value _avio)
{
  ...
  if( ! av) {
    ...
    ocaml_avutil_raise_error(lasterr);
  }
...
static int decode_frame(av_t * av, stream_t * stream, AVPacket * packet, AVFrame * frame, value *frame_kind)
{
...
CAMLprim value ocaml_av_read_stream_packet(value _stream) {
  ...
  if(av->release_out) {
    if( ! alloc_packet_value(&packet_value)) ocaml_avutil_raise_error(lasterr);
  }
  else {
  ...
  if (ret < 0) Raise(EXN_FAILURE, "Failed to read packet : %s", av_err2str(ret));
...
CAMLprim value ocaml_av_read_stream_frame(value _stream) {
  ...
  for(; ! frame_kind;) {
    ...
    ret = decode_frame(av, stream, packet, frame, &frame_kind);
    if (ret < 0) break; 
  }
  caml_acquire_runtime_system();
...
CAMLprim value ocaml_av_new_audio_stream(value _av, value _audio_codec_id, value _channel_layout, value _sample_fmt, value _bit_rate, value _sample_rate, value _time_base)
{
  ...
  if( ! stream) ocaml_avutil_raise_error(lasterr);
...
CAMLprim value ocaml_av_write_stream_frame(value _stream, value _frame)
{
  ...
  if( ! av->streams) ocaml_avutil_raise_error(lasterr);
