...
external get_all_filters : unit -> ([`Unattached], 'a, 'c) _filter array = "ocaml_avfilter_get_all_filters" 
...
let attach ?args ~name filter graph =
  ...
  in
  let f () =
    List.map (attach_pad filter_ctx graph)
  in
  let inputs = {
...
type 'a parse_io =
  (('a, [`Input])  parse_av,
   ('a, [`Output])  parse_av) io

external parse : inputs:(string*filter_ctx*int) array -> outputs:(string*filter_ctx*int) array -> string -> _config -> unit = "ocaml_avfilter_parse"

let parse ({inputs;outputs}:[`Unattached] parse_io) filters graph =
  let get_pad
     (type a) (type b)
     (node:([`Unattached], a, b) parse_node) : ([`Unattached], a, b) parse_node*filter_ctx =
    let {node_name;node_args;node_pad} = node in
    if List.mem node_name graph.names then
      raise Exists;
    let {filter_name} = node_pad in
  ...
  in
  let audio_inputs =
    List.map get_pad inputs.audio
  in
  let video_inputs =
    List.map get_pad inputs.video
  in
  let audio_outputs =
    List.map get_pad outputs.audio
  in
  let video_outputs =
    List.map get_pad outputs.video
  in
  let get_ctx ({node_name;node_pad}, filter_ctx) =
    node_name, filter_ctx, node_pad.idx
  in
  let inputs =
    Array.of_list
      ((List.map get_ctx audio_inputs) @
       (List.map get_ctx video_inputs))
  in
  let outputs =
    Array.of_list
      ((List.map get_ctx audio_outputs) @
       (List.map get_ctx video_outputs))
  in
  parse ~inputs ~outputs filters graph.c;
  let attach_pad
    (type a)
    (type b)
    ((node:([`Unattached], a, b) parse_node), filter_ctx) : ([`Attached], a, b) parse_node =
