...
CAMLprim value ocaml_avcodec_packet_content(value _packet) {
    ...
    CAMLreturn(
        caml_alloc_initialized_string(packet->size, (char *)packet->data));
}
...
CAMLprim value ocaml_avcodec_packet_side_data(value _packet) {
    ...
    for (i = 0; i < len; i++) {
        ...
        case AV_PKT_DATA_STRINGS_METADATA:
            ...
            tmp = caml_alloc_initialized_string(
                packet->side_data[i].size, (char *)packet->side_data[i].data);
...
CAMLprim value ocaml_avcodec_create_decoder(value _params, value _codec) {
    ...
    codec_context_t *ctx =
        (codec_context_t *)av_mallocz(sizeof(codec_context_t));
    if (!ctx)
        caml_raise_out_of_memory();

    ans =
        caml_alloc_custom(&codec_context_ops, sizeof(codec_context_t *), 0, 1);
...
static void send_frame(codec_context_t *ctx, AVFrame *frame) {
    ...
    if (ctx->codec_context->hw_frames_ctx && frame) {
        ...
        ret = av_hwframe_get_buffer(ctx->codec_context->hw_frames_ctx, hw_frame,
                                    0);
...
CAMLprim value ocaml_avcodec_params_descriptor(value _params) {
    CAMLparam1(_params);
    CAMLreturn(
        ocaml_avcodec_descriptor(CodecParameters_val(_params)->codec_id));
}
...
CAMLprim value ocaml_avcodec_get_supported_channel_layouts(value _codec) {
    ...
    err = avcodec_get_supported_config(NULL, codec,
...
CAMLprim value ocaml_avcodec_get_supported_sample_rates(value _codec) {
    ...
    err = avcodec_get_supported_config(NULL, codec, AV_CODEC_CONFIG_SAMPLE_RATE,
...
CAMLprim value ocaml_avcodec_parameters_get_sample_format(value _cp) {
    CAMLparam1(_cp);
    CAMLreturn(Val_SampleFormat(
        (enum AVSampleFormat)CodecParameters_val(_cp)->format));
}
...
CAMLprim value ocaml_avcodec_get_supported_frame_rates(value _codec) {
    ...
    err = avcodec_get_supported_config(NULL, codec, AV_CODEC_CONFIG_FRAME_RATE,
                                       0, (const void **)&supported_framerates,
                                       NULL);
...
CAMLprim value ocaml_avcodec_get_supported_pixel_formats(value _codec) {
    ...
    err = avcodec_get_supported_config(NULL, codec, AV_CODEC_CONFIG_PIX_FORMAT,
...
CAMLprim value ocaml_avcodec_parameters_get_pixel_aspect(value _cp) {
    ...
    const AVRational pixel_aspect =
        CodecParameters_val(_cp)->sample_aspect_ratio;
...
CAMLprim value ocaml_avcodec_parameters_video_copy_byte(value *argv, int argn) {
    return ocaml_avcodec_parameters_video_copy(
        argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[7]);
}
...
CAMLprim value ocaml_avcodec_get_unknown_codec_id_name(value _codec_id) {
    CAMLparam1(_codec_id);
    CAMLreturn(
        caml_copy_string(avcodec_get_name(UnknownCodecID_val(_codec_id))));
}
