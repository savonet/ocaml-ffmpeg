...
int subtitle_header_default(AVCodecContext *avctx) {
  // Generate a default ASS header for subtitle encoders that require it
  // (like subrip, webvtt, etc. which use ff_ass_split internally).
  // Based on FFmpeg's ff_ass_subtitle_header_full from libavcodec/ass.c
  static const char ass_header_template[] =
      "[Script Info]\r\n"
  ...
  size_t header_size = sizeof(ass_header_template);
  avctx->subtitle_header = av_mallocz(header_size);
  if (!avctx->subtitle_header)
    return AVERROR(ENOMEM);

  memcpy(avctx->subtitle_header, ass_header_template, header_size);
  avctx->subtitle_header_size = header_size - 1; // exclude null terminator
