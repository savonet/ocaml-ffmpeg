open FFmpeg.Avfilter

let () =
  Printexc.record_backtrace true

let string_of_pad {audio;video} =
  let audio = String.concat "\n" (List.map (fun pad ->
    Printf.sprintf "- name: %s, type: audio" (pad_name pad)) audio)
  in

  let video = String.concat "\n" (List.map (fun pad ->
    Printf.sprintf "- name: %s, type: video" (pad_name pad)) video)
  in
