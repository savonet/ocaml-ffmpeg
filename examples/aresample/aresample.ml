...
let () =
  ...
  let filter =
    ...
    let _ =
      Avfilter.parse {inputs;outputs} "aresample=22050,aformat=channel_layouts=stereo" config
    in
  ...
  let process_audio i frm =
   try
    ...
    let _, input =
      List.hd Avfilter.(filter.inputs.audio)
    in
    input frm;
    let _, output =
      List.hd Avfilter.(filter.outputs.audio)
    in
    let rec flush () =
      try
